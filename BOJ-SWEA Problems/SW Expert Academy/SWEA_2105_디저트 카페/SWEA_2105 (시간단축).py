...
def DFS(n, ci, cj, v, cnt):
    ...
    if n > 3:
        return
    if ci == si and cj == sj and n == 3 and ans < cnt:                          ### 가능성이 희박한 조건을 앞으로 이동시켜서 시간 단축
        ans = cnt
        return
    ...
    for k in range(n, n+2):
        ...
        if 0 <= ni < N and 0 <= nj < N and num_mat[ni][nj] not in v:
            v.append(num_mat[ni][nj])                                           ### append 사용으로 시간 단축
            DFS(k, ni, nj, v, cnt+1)
            v.pop()
...
for tc in range(1, T+1):
    ...
    ans = -1
    for si in range(0, N-2):                                                    ### 범위를 실용적인 범위로 좁혀줌으로 시간 단축
        for sj in range(1, N-1):                                                ### 범위를 실용적인 범위로 좁혀줌으로 시간 단축
