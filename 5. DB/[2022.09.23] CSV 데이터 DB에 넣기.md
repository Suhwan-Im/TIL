...
![ZZIKMUK ERD](C:\Users\SSAFY\Desktop\ZZIKMUK ERD.png)
...
with open(CSV_PATH_PRODUCTS, 'rt', encoding='UTF8') as in_file:
    ...
    for row in data_reader:
        print(row[0])
        id_csv = row[0]
        recipe_id_csv = row[1]		# foreign key 값을 csv에서 가져온 뒤
        recipe_id_pk = Recipe.objects.get(id=recipe_id_csv)	# 참조 테이블에 해당 값을 검색
...
- 참고문헌: https://codermun-log.tistory.com/141